...
@broker.subscriber(settings.consultation_request_queue)
async def handle_consultation_request(message: object) -> None:
    if _bot is None:
        logger.error("Bot instance is not configured for consultation request consumer")
        return
    ...
    for chat_id in settings.chat_ids_list:
        try:
            ...
            logger.exception(
                "Failed to send consultation request notification to chat_id=%s",
                chat_id,
            )
